...
- **Given** the `notes_system` and `video_analysis` schemas, **When** multiple concurrent operations (create, read, update, delete) are executed on both schemas, **Then** there is no data corruption, no cross-schema lock conflicts, and transaction isolation is maintained without performance degradation.
...
- **Given** various error conditions (e.g., transaction failure, constraint violation, deadlocks), **When** these errors occur in one schema, **Then** the error does not propagate to the other schema, and rollbacks are schema-specific.
...
- **Given** operations that span both schemas, **When** these transactions are executed, **Then** ACID properties are maintained across both schemas.
...
### Edge Cases
- What happens when there is a deadlock between two transactions in different schemas?
- How does the system handle a loss of connection during a long-running operation in one schema?
...
- **`notes_system` schema**: Contains all data related to the notes system, including notes, tags, attachments, and versions.
- **`video_analysis` schema**: Contains all data related to video analysis, including videos, transcripts, analysis results, and metadata.
